...
/**
 * Main entry point for the SobySequencer application.
 * Runs warmup and benchmark phases, printing latency reports.
 */
public class Main {
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("[SobySequencer] Warmup: " + config.getWarmupPublishCount() + " events published");
            long warmupStart = System.nanoTime();
            for (long i = 0; i < config.getWarmupPublishCount(); i++) {
                producer.publishOrder(i);
            }
            long warmupEnd = System.nanoTime();
            double warmupRate = config.getWarmupPublishCount() / ((warmupEnd - warmupStart) / 1_000_000_000.0);
            System.out.printf("[SobySequencer] Warmup completed at %.2f events/sec%n", warmupRate);

            // Benchmark phase - record latencies
            System.out.println("[SobySequencer] Benchmark: " + config.getBenchmarkPublishCount() + " events published");
            ...
            double benchmarkRate = config.getBenchmarkPublishCount() / ((benchmarkEnd - benchmarkStart) / 1_000_000_000.0);
            ...
            System.out.println("[SobySequencer] Total throughput: " + String.format("%.2f", benchmarkRate) + " events/sec");
