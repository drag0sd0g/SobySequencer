...
/**
 * Handler that outputs the final event state. In a real system this would publish execution reports
 * back to clients via FIX protocol. Currently logs the event state via SLF4J.
 */
public class OutputHandler implements EventHandler<OrderEvent> {
  ...
  public void onEvent(OrderEvent event, long sequenceNumber, boolean endOfBatch) {
    ...
    if (LOG.isTraceEnabled()) {
      LOG.trace(
          "Event {}: order={}, symbol={}, side={}, type={}, price={}, qty={}, state={}",
          sequenceNumber,
          event.getOrderId(),
          event.getSymbol(),
          event.getSide(),
