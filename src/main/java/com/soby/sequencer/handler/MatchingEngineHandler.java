...
/**
 * Handler that matches orders in an in-memory order book. Maintains two price-time priority order
 * books: bids (BUY) and asks (SELL). Uses TreeMap for price-level ordering and ArrayDeque for FIFO
 * within each level. No allocations on the matching path - uses pre-allocated result objects.
 */
public class MatchingEngineHandler implements EventHandler<OrderEvent> {
  ...
  private void executeMatch(OrderEvent buyer, OrderEvent seller, long quantity, long price) {
    ...
    // LOG.debug("MATCH: Buy={} Sell={} Qty={} Price={}", buyer.getOrderId(), seller.getOrderId(),
    // quantity, price);
  }
