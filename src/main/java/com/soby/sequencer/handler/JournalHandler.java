...
/**
 * Handler that writes events to a memory-mapped file journal for durability.
 * Uses a pre-allocated 64MB mapped file for zero-copy journaling with fixed-width binary format.
 * Format: sequenceNumber (8 bytes) + orderId (8 bytes) + price (8 bytes) + quantity (8 bytes) + side (1 byte) + type (1 byte) = 34 bytes per entry
 */
public class JournalHandler implements EventHandler<OrderEvent>, AutoCloseable {
    ...
    private static final int ENTRY_SIZE = 34; // sequenceNumber (8) + orderId (8) + price (8) + quantity (8) + side (1) + type (1)
     ...
     */
    public JournalHandler(String journalFilePath, LatencyRecorder latencyRecorder) throws IOException {
    ...
    /**
     * Write an event to the journal. Records latency from event timestamp to journal write completion.
    ...
    /**
     * Write an event to the journal. Records latency from event timestamp to journal write completion.
     * Convenience method that extracts data directly from event fields.
     ...
     * @return the journal position after writing
     */
    public long journal(long sequenceNumber, long orderId, long price, long quantity, byte side, byte type) {
        long startTime = System.nanoTime();
