...
/**
 * Produces synthetic orders for testing the sequencer.
 * Uses ThreadLocalRandom for lock-free random number generation on the hot path.
 * Records end-to-end latency for benchmarking.
 */
public class OrderProducer {
    ...
    /**
     * Create a random order with synthetic data.
     * Uses ThreadLocalRandom for zero-contention random number generation.
    ...
    private Order createRandomOrder(long orderId) {
        ...
        long quantity = QUANTITY_MIN + ThreadLocalRandom.current().nextLong(QUANTITY_MAX - QUANTITY_MIN + 1);
